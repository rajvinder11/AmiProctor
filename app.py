...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'dummyekey'#
app.config['MYSQL_HOST'] = 'localhost'
app.config['MYSQL_USER'] = 'root'
app.config['MYSQL_PORT'] = 3306
app.config['MYSQL_PASSWORD'] = 'R@jvinder#11'
app.config['MYSQL_DB'] = 'quizapp'
app.config['MYSQL_CURSORCLASS'] = 'DictCursor'

app.config['MAIL_SERVER']='smtp.gmail.com'
app.config['MAIL_PORT'] = 587
app.config['MAIL_USERNAME'] = 'amiproct.ai@gmail.com'
app.config['MAIL_PASSWORD'] = 'osgmjogeivhgajom'
...
app.secret_key= 'internshiproject'

mysql = MySQL(app)

sender = 'amiproct.ai@gmail.com'
...
def contact():
	if request.method == 'POST':
		careEmail = "careamiproctor.ai@gmail.com"
...
def register():
	if request.method == 'POST':
		...
		msg1 = Message('AmiProctor.ai - OTP Verification', sender = sender, recipients = [email])
...
def verifyEmail():
	dbName = session.get('tempName', None)
	print(dbName)
	if request.method == 'POST':
		print(session)
...
def create_test_lqa():
	...
	if request.method == 'POST' and form.validate_on_submit():
		...
		if ecc:
			...
			flash("Exam credits points are Updated! Please try again!")
			cur.execute('UPDATE users set examcredits = examcredits+10 where email = %s and uid = %s', (session['email'], session['uid']))
			return redirect(url_for('professor_index'))
